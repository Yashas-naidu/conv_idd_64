...
# checkpoint_path = "/home/jovyan/AV_GS4/conv_idd_64/save_model/2020-03-09T00-00-00/checkpoint_0_0.000343.pth.tar"
...
# # Prepare the data
# dataset_root = "/mnt/local/gs_datasets/idd-train-set4/idd_temporal_train_4"  # Path to the dataset root folder
...
output_dir = "/home/jovyan/AV_GS4/conv_idd_64"
...
# Load the checkpoint
checkpoint_path = "/home/jovyan/AV_GS4/conv_idd_64/save_model/2020-03-09T00-00-00/checkpoint_14_0.000318.pth.tar"
checkpoint = torch.load(checkpoint_path, map_location=torch.device('cpu') if not torch.cuda.is_available() else None)
...
# Make predictions and save input vs predicted vs ground truth frames
with torch.no_grad():
    for batch_idx, (idx, targetVar, inputVar, frozen, _) in enumerate(validLoader):
        if batch_idx >= num_batches_to_process:
            break
        ...
        for sample_idx in range(min(num_samples_per_batch, pred.shape[0])):
            ...
            for i in range(inputs.shape[1]):
                plt.subplot(3, 2, i + 1)
                frame = inputs[sample_idx, i, :, :, :]
                frame = (frame - frame.min()) / (frame.max() - frame.min())
                plt.imshow(frame.transpose(1, 2, 0))
                plt.title(f'Input Frame {i * 5 + 1}')
                plt.axis('off')

            # Display the predicted frame (Row 2)
            plt.subplot(3, 2, 3)
            frame = pred[sample_idx, 0, :, :, :]
            frame = (frame - frame.min()) / (frame.max() - frame.min())
            plt.imshow(frame.transpose(1, 2, 0))
            ...
            output_path = os.path.join(output_dir, f'prediction_batch_{batch_idx}_sample_{sample_idx}.png')
